...
def show_analysis_page():
    ...
    with tab1:
        ...
        # 디버그 정보 표시
        if not df.empty:
            st.info(f"📊 로드된 데이터: {len(df)}행, 컬럼: {list(df.columns)}")
            # 선택한 연도의 데이터 샘플 표시
            with st.expander("🔍 선택한 연도 데이터 미리보기"):
        ...
        # 데이터 필터링 (선택한 연도의 모든 지역 데이터 표시)
        filtered_df = df[df['연도'] == selected_year].copy()
        
        # 연도 컬럼 제거 (드롭다운에서 이미 선택했으므로)
        ...
        # 선택한 연도의 전체 데이터 표시
        st.markdown(f"#### 📋 {selected_year}년 전체 지역 데이터")
        ...
        if not filtered_df.empty:
            ...
            with col1:
                total_regions = len(display_df)
                st.metric("총 지역 수", f"{total_regions}개")
            with col2:
    ...
    with tab2:
        ...
        **분석 조건:**
        ...
        # 구급차 분석을 위한 연도 선택 (앞에서 선택한 연도와 동기화)
        col1, col2 = st.columns([1, 3])
        with col1:
